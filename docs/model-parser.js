...
// ═══════════════════════════════════════════════════════════════════════════════
export function findSemanticModelPath(reportPath) {
 ...
 */
function extractTmdlPartitions(content, expressions = []) {
    ...
    for (const e of expressions)
        expressionByName.set(e.name, e.value);
    let current = null;
    let inSource = false;
    const flush = () => {
        if (!current)
 ...
 *     ...
 */
function extractTmdlHierarchies(content) {
    const lines = content.split("\n");
    const out = [];
    ...
    const flushLevel = () => {
        if (current && currentLevel)
            current.levels.push(currentLevel);
        currentLevel = null;
    };
...
function parseTmdlExpressions(modelPath) {
    ...
    for (const line of lines) {
        ...
        if (current) {
            if (/^lineageTag:/.test(trimmed) || /^annotation\b/.test(trimmed) || /^changedProperty\b/.test(trimmed))
                continue;
            if (collectingValue && trimmed)
...
function parseTmdlDatabaseLevel(modelPath) {
 ...
 */
function parseTmdlModelProperties(modelPath) {
    const props = {
        name: "",
        description: "",
        culture: "",
            ...
            if (tabCount >= 2) {
                ...
                if (isProp) {
                    ...
                    if (propLine.startsWith("formatString:")) {
                        ...
                        if (currentMeasure)
                            currentMeasure.formatString = fmt;
                        else if (currentColumn)
                            currentColumn.formatString = fmt;
                    }
                    ...
                    if (propLine.startsWith("description:")) {
                        ...
                        if (currentMeasure && !currentMeasure.description)
                            currentMeasure.description = desc;
                        else if (currentColumn && !currentColumn.description)
                            currentColumn.description = desc;
                    }
                    if (propLine.startsWith("displayFolder:")) {
                        const folder = propLine.replace("displayFolder:", "").trim().replace(/^['"]|['"]$/g, "");
                        if (currentMeasure)
                            currentMeasure.displayFolder = folder;
                        else if (currentColumn)
                            currentColumn.displayFolder = folder;
                    }
                    continue;
                }
...
function parseBimFile(bimPath) {
    ...
    for (const table of bim.model?.tables || []) {
        ...
        const tablePartitions = (table.partitions || []).map((p) => {
            const src = p?.source || {};
            ...
            const partitionKind = typeof src.type === "string" ? src.type : "";
            ...
            const expressionSource = typeof src.expressionSource === "string"
        ...
        const tableHierarchies = (table.hierarchies || []).map((h) => ({
            name: h.name || "",
        ...
        for (const c of table.columns || []) {
            ...
            const extProps = Array.isArray(c.extendedProperties) ? c.extendedProperties : [];
            const hasParameterMetadata = extProps.some((p) => p && p.name === "ParameterMetadata");
            columns.push({
                name: c.name,
                table: tableName,
    ...
    const modelProperties = {
        ...
        cultures: Array.isArray(bm.cultures)
            ? bm.cultures.map((c) => c?.name).filter((n) => typeof n === "string").sort()
            : [],
...
export function parseModel(modelPath) {
    const tablesDir = path.join(modelPath, "definition", "tables");
