...
function statusLabel(s) {
    if (s === "direct")
        return '<span class="badge badge--success">✓ Direct</span>';
    if (s === "indirect")
        return '<span class="badge badge--indirect">↻ Indirect</span>';
    if (s === "unused")
 ...
 */
function mermaidMeasureLineage(m) {
    const upstream = m.daxDependencies ?? [];
    const downstream = m.usedIn ?? [];
    const downByTitle = new Map();
    for (const u of downstream) {
        const key = u.visualTitle || u.visualType || u.visualId;
        if (!downByTitle.has(key))
            downByTitle.set(key, { title: key, type: u.visualType });
    }
    const downVisuals = [...downByTitle.values()];
    if (upstream.length === 0 && downVisuals.length === 0)
        return "";
 ...
 */
function mermaidTableRelationships(t) {
...
function isAutoDate(t) { return t.origin === "auto-date"; }
...
// ═══════════════════════════════════════════════════════════════════════════════
export function generateMarkdown(data, reportName) {
    ...
    for (const t of tables)
        rolesByTable.set(t.name, classifyTable(t));
    const roleCounts = {
    ...
    for (const t of userTablesSorted)
        roleCounts[rolesByTable.get(t.name)]++;
...
//   <details> per measure.
// ═══════════════════════════════════════════════════════════════════════════════
export function generateMeasuresMd(data, reportName) {
    const ts = new Date().toISOString().replace("T", " ").substring(0, 16);
    const lines = [];
    const t = data.totals;
    // ── Front matter ──────────────────────────────────────────────────────────
    ...
    buckets.set("#", []);
    for (const m of data.measures)
        buckets.get(bucketLetter(m.name)).push(m);
    ...
    lines.push("");
    // ── Sections ──────────────────────────────────────────────────────────────
    const renderSection = (heading, _anchor, items) => {
        ...
        for (const m of items) {
            ...
            const statusTag = isProxy ? " · _external proxy_"
            ...
            if (m.externalProxy) {
                ...
                if (p.cluster)
                    lines.push(`| AS cluster | \`${esc(p.cluster)}\` |`);
    ...
    for (const L of letters)
        renderSection(L, L.toLowerCase(), buckets.get(L));
    ...
    if (buckets.get("#").length > 0)
        renderSection("Other", "other", buckets.get("#"));
...
// ═══════════════════════════════════════════════════════════════════════════════
export function generateFunctionsMd(data, reportName) {
    const ts = new Date().toISOString().replace("T", " ").substring(0, 16);
    const lines = [];
    ...
    const refsFor = (name) => data.measures.filter(m => m.daxExpression.includes("'" + name + "'") || m.daxExpression.includes(name + "("));
    for (const f of fns) {
...
// ═══════════════════════════════════════════════════════════════════════════════
export function generateCalcGroupsMd(data, reportName) {
    const ts = new Date().toISOString().replace("T", " ").substring(0, 16);
    const lines = [];
...
// ═══════════════════════════════════════════════════════════════════════════════
export function generateQualityMd(data, reportName) {
    const ts = new Date().toISOString().replace("T", " ").substring(0, 16);
    const lines = [];
    ...
    const isAutoDateCol = (c) => autoDateTableNames.has(c.table);
    ...
    if (unusedM.length === 0 && unusedC.length === 0 && proxyM.length === 0 && unusedC_autoDate.length === 0) {
        ...
        if (unusedC.length > 0) {
            ...
            [...unusedC].sort((a, b) => a.table.localeCompare(b.table) || a.name.localeCompare(b.name)).forEach(c => {
                const notes = [];
                if (c.isHidden)
                    notes.push(BADGE_HIDDEN);
                if (c.isCalculated)
                    notes.push(BADGE_CALC);
                if (c.isKey)
                    notes.push(BADGE_PK);
...
// ═══════════════════════════════════════════════════════════════════════════════
export function generateDataDictionaryMd(data, reportName) {
    const ts = new Date().toISOString().replace("T", " ").substring(0, 16);
    const lines = [];
    ...
    for (const tbl of userTablesSorted) {
        ...
        if (tbl.columns.length === 0) {
            ...
            tbl.columns.forEach((c, i) => {
                const constraints = [];
                if (c.isKey)
                    constraints.push(BADGE_PK);
                else if (c.isInferredPK)
                    constraints.push(BADGE_PK_INF);
                if (c.isFK && c.fkTarget)
                    constraints.push(`${BADGE_FK} → ${c.fkTarget.table}[${c.fkTarget.column}]`);
                ...
                if (c.isCalculated)
                    constraints.push(BADGE_CALC);
                if (c.isHidden)
                    constraints.push(BADGE_HIDDEN);
