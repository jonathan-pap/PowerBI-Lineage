import { findSemanticModelPath, parseModel } from "./model-parser.js";
...
// ═══════════════════════════════════════════════════════════════════════════════
export function buildFullData(reportPath) {
    ...
    const EXTERNAL_MEASURE_RX = /EXTERNALMEASURE\s*\(\s*"((?:[^"]|"")*)"\s*,\s*(\w+)\s*,\s*"DirectQuery to AS - ((?:[^"]|"")+)"\s*\)/i;
    const undoubleDaxQuotes = (s) => s.replace(/""/g, '"');
    // Build measures
    const measures = rawModel.measures.map(m => {
        ...
        const externalProxy = extMatch
            ? {
                ...
                cluster: expressionClusterByName.get(`DirectQuery to AS - ${undoubleDaxQuotes(extMatch[3])}`) ?? null,
            }
            : null;
        return {
            name: m.name,
    ...
    const tableDescByName = new Map();
    const tablePartitionsByName = new Map();
    const tableHierarchiesByName = new Map();
    ...
    const parameterMetadataTables = new Set();
    for (const c of rawModel.columns) {
        if (c.hasParameterMetadata)
            parameterMetadataTables.add(c.table);
    }
    // Calc groups also expose descriptions on their own entity.
    for (const cg of rawModel.calcGroups) {
        if (cg.description && !tableDescByName.get(cg.name))
            tableDescByName.set(cg.name, cg.description);
    }
    const tables = [...tableNames].sort((a, b) => a.localeCompare(b)).map(tableName => {
        ...
        // are auto-date noise.
        const isAutoDate = /^LocalDateTable_/.test(tableName) || /^DateTableTemplate_/.test(tableName);
        ...
        const isCalculatedTable = !calcGroupNames.has(tableName) &&
        ...
        const hasDirectQueryEntity = tablePartitions.some(p => p.mode === "directQuery" && !!p.expressionSource);
        const singleSelfColumn = tableColumns.length === 1 && tableColumns[0].name === tableName;
        const isCompositeModelProxy = hasDirectQueryEntity && singleSelfColumn;
        const parameterKind = parameterMetadataTables.has(tableName) ? "field"
        ...
        return {
            ...
            origin: isAutoDate ? "auto-date" : "user",
            isCalculatedTable,
            parameterKind,
