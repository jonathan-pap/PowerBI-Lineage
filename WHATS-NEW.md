...
- **Unused** — orphan measures, dead-chain measures, indirect-use detection. When measures are flagged, a top toolbar generates a **cleanup export** in your choice of format: an **AI prompt** (paste into Claude Code with the [`pbi-desktop`](https://github.com/data-goblin/power-bi-agentic-development) plugin or any AI agent that can drive TOM / Tabular Editor CLI), or a **Tabular Editor script** (paste-into-TE2/TE3 C# `.csx`, F5 to run). PowerBI-Lineage never deletes anything itself — it hands off the ready-to-run artefact with Stage 1 / Stage 2 ordering + EXTERNALMEASURE safety guards baked in.
...
- **Zero runtime dependencies**, MIT-licensed, 311 tests
