...
export function join(...segments: string[]): string {
  ...
  for (const s of segments) {
    ...
    if (normalised.startsWith("/") && parts.length === 0) absolute = true;
    for (const p of normalised.split("/")) parts.push(p);
  }
...
export function resolve(...segments: string[]): string {
  ...
  for (const s of segments) {
    if (!s) continue;
    const normalised = s.replace(/\\/g, "/");
    if (normalised.startsWith("/")) resolvedPath = normalised;
    else resolvedPath = resolvedPath + "/" + normalised;
  }
...
export default {
  ...
  extname,
  sep,
  posix: undefined as never,   // intentionally unavailable — throw if used
  win32: undefined as never,
};
